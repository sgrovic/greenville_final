...
function addActivity($connection, $data) {
    $stmt = $connection->prepare("CALL AddActivity(?,?,?)");
    $stmt->bind_param("sss",
        $data['username'],
        $data['date1'],
        $data['action1']
...
function listActivitybyDate($connection, $data) {
    $stmt = $connection->prepare("CALL GetActivityByDate(?,?)");
    $stmt->bind_param(
...
switch ($method) {
    case "add":
        addActivity($connection, $data);
        break;
    case "listactivitybydate":
        listActivitybyDate($connection, $data);
        break;
