...
function createSubject($connection, $data) {
    $stmt = $connection->prepare("CALL AddSubject(?,?,?,?)");
...
function searchSubjectsByProgram($connection, $data) {
    //$stmt = $connection->prepare("CALL SearchStudentByLastName(?)");
    $stmt = $connection->prepare("CALL SearchSubjectsByProgram(?)");
    $stmt->bind_param("s", $data['program']);
    $stmt->execute();
    $result = $stmt->get_result();
...
switch ($method) {
    case "create":
        createSubject($connection, $data);
        break;

    case "update":
        updateSubject($connection, $data);
        break;
    ...
    case "searchsubjectbyprogram":
        searchSubjectsByProgram($connection, $data);
    break;
